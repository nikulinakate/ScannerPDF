...
class PDFConvertViewModel: ObservableObject {
    ...
    private func convertToText(document: PDFDocument, pages: [Int]) async throws -> URL {
        ...
        let tempDir = FileManager.default.temporaryDirectory
        let outputURL = tempDir.appendingPathComponent("extracted_text_\(UUID().uuidString).txt")
        
        try extractedText.write(to: outputURL, atomically: true, encoding: .utf8)
        progress = 1.0
    ...
    private func convertToPowerPoint(document: PDFDocument, pages: [Int]) async throws -> URL {
        ...
        let tempDir = FileManager.default.temporaryDirectory
        let outputURL = tempDir.appendingPathComponent("presentation_\(UUID().uuidString).html")
    ...
    private func createZipArchive(from directory: URL) async throws -> URL {
        ...
        // In production, implement proper ZIP creation using ZIPFoundation
        return directory
    }
}
